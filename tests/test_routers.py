...
class TestProfileRouter:
    ...
    def test_dietary_restrictions_round_trip(self, client):
        r = client.put("/api/profile", json={"dietary_restrictions": ["vegan", "gluten-free"]})
        assert r.status_code == 200
    ...
    def test_goal_change_recalculates_targets(self, client):
        ...
        maintain_cal = client.get("/api/profile").json()["calorie_target"]

        r = client.put("/api/profile", json={"goal": "bulk"})
    ...
    def test_partial_metrics_dont_crash(self, client):
        ...
        r = client.put("/api/profile", json={"weight_kg": 80})
        assert r.status_code == 200
...
class TestWeightLogRouter:
    def test_upsert_and_list(self, client):
        client.post("/api/weight-log", json={"date": "2026-06-01", "weight_kg": 80.5})
        client.post("/api/weight-log", json={"date": "2026-06-02", "weight_kg": 80.2})
        rows = client.get("/api/weight-log").json()
        assert len(rows) == 2
        assert rows[0]["date"] == "2026-06-02"  # DESC order

    def test_upsert_replaces_same_date(self, client):
        client.post("/api/weight-log", json={"date": "2026-06-01", "weight_kg": 80.0})
        client.post("/api/weight-log", json={"date": "2026-06-01", "weight_kg": 79.0})
        rows = client.get("/api/weight-log").json()
        assert len(rows) == 1
    ...
    def test_delete(self, client):
        client.post("/api/weight-log", json={"date": "2026-06-01", "weight_kg": 80.0})
        r = client.delete("/api/weight-log/2026-06-01")
        assert r.status_code == 204
        assert client.get("/api/weight-log").json() == []


class TestMeasuredTdee:
    def test_returns_none_with_fewer_than_two_weighins(self, client):
        client.post("/api/weight-log", json={"date": "2026-06-01", "weight_kg": 80.0})
        assert client.get("/api/weight-log/measured-tdee").json() is None

    def test_returns_none_when_window_under_7_days(self, client):
        client.post("/api/weight-log", json={"date": "2026-06-01", "weight_kg": 80.0})
        client.post("/api/weight-log", json={"date": "2026-06-05", "weight_kg": 79.5})
        assert client.get("/api/weight-log/measured-tdee").json() is None

    def test_returns_none_with_under_7_tracked_days(self, client, seed_eaten_day):
        client.post("/api/weight-log", json={"date": "2026-06-01", "weight_kg": 80.0})
        client.post("/api/weight-log", json={"date": "2026-06-15", "weight_kg": 79.0})
    ...
    def test_computes_measured_tdee(self, client, seed_eaten_day):
        ...
        body = client.get("/api/weight-log/measured-tdee").json()
